...
homerange %>% 
  ...
  geom_histogram(alpha = 0.4, color = "pink", fill = "lightskyblue1", bins=40)+
...
homerange %>% 
  ggplot(aes(x = log10.mass)) +
  geom_density(fill="deepskyblue1", alpha  =0.4, color = "black")+
  labs(title = "Distribution of Body Mass")
```
...
```{r}
homerange%>%
  ggplot(aes(x=log10.hra)) +geom_histogram(fill="powderblue", color="pink") +labs(title="Homerange Distribution")
``` 

2. Now plot the same variable using `geom_density()`.
```{r}
homerange%>%
  ggplot(aes(x=log10.hra)) + geom_density(fill="lightblue", color="pink") + labs(title="Homerange Distribution")
```
...
homerange %>% 
  ...
  geom_histogram(aes(y = ..density..), fill = "lavender", color = "pink")+
...
homerange <- homerange%>%
  mutate(range_category=case_when(log10.hra <=3.653 ~ "small", 
                                  log10.hra>3.653 & log10.hra<=6.016 ~"medium", 
...
homerange%>%
  ...
  ggplot(aes(x=log10.mass, fill=taxon)) + geom_histogram() + labs(title="Small Mass Range")
```
