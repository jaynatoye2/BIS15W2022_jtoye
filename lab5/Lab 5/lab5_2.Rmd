...
```
```{r}
bald_superhero <- superhero_info%>%
  select(alignment, hair_color)%>%
  filter(alignment=="good" | alignment=="bad", hair_color=="No Hair")
bald_superhero
```
```{r}
table(bald_superhero)
```
There are more good that are bald but are almost even.
...
superhero_info%>%
  ...
  filter(weight>450)
```
I think there aren't only 16 rows in #10 because we included superheros with a height over 300 regardless if their weight is over 450 and vice versa. 
## Height to Weight Ratio
13. It's easy to be strong when you are heavy and tall, but who is heavy and short? Which superheros have the highest height to weight ratio?
```{r}
superhero_info%>%
  select(height, weight, name)%>%
  filter(height<70, weight>450)
```
