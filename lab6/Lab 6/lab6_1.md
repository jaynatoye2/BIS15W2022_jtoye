...
##    name            order    bodywt
##    <chr>           <chr>     <dbl>
##  1 Owl monkey      Primates  0.48 
##  2 Grivet          Primates  4.75 
##  3 Patas monkey    Primates 10    
##  4 Galago          Primates  0.2  
##  5 Human           Primates 62    
##  6 Mongoose lemur  Primates  1.67 
##  7 Macaque         Primates  6.8  
##  8 Slow loris      Primates  1.4  
##  9 Chimpanzee      Primates 52.2  
## 10 Baboon          Primates 25.2  
## 11 Potto           Primates  1.1  
## 12 Squirrel monkey Primates  0.743
```
...
```r
msleep%>%
  summarise(mean_sleep = mean(sleep_total), min_sleep = min(sleep_total), max_sleep = max(sleep_total), total = n())
```
...
msleep%>%
   group_by(order) %>% 
  summarize(mean_braintw = mean(brainwt))
```
...
Though there are some NA in the order not all of them are NA so we haven't filtered out the NA yet. 
...
msleep%>%
   group_by(order) %>% 
  summarize(mean_braintw = mean(brainwt, na.rm=TRUE))
```
...
We don't have any data on Cetacea! 
