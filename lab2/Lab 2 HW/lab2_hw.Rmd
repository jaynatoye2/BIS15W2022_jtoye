...
A vector is a way of organizing data by assigning values to objects.
2. What is a data matrix in R?  
A data matrix is a series of stacked vectors resembling data tables. 
...
```{r}
mean_spring_temp <-mean(spring_temp)
mean_spring_temp
```

7. Add this as a new column in the data matrix.  
```{r}
rbind(spring_temp_matrix, mean_spring_temp)
```
...
```{r}
Jill_values <- spring_temp_matrix[,1]
mean(Jill_values)
```

10. Use the data matrix to perform one calculation or operation of your interest.
```{r}
Steve_sum_temp<- spring_temp_matrix[2:8]
Steve_sum_temp
sum(Steve_sum_temp)
```
