...
```{r}
homerange%>%
  ggplot(aes(x=log10.mass, y=taxon, fill=taxon))+ geom_boxplot(color="lightblue", fill="gray", alpha=.4)+ geom_violin(alpha=.5) 
```
...
heartrate_diff %>% 
  ggplot(aes(patient, drug, fill= heartrate)) + 
  geom_tile()+
  scale_fill_gradient2(low="pink", mid = "white", high="blue")
...
penguins%>%
  ...
  ggplot(aes(x=species, y=sex, fill=body_mass_g)) + geom_tile() + scale_fill_gradient(low="pink", high="blue")
```
